...
class UBXTracker:
    ...
    def reader(self):
        """Reads and parses UBX message data from stream using UBXReader iterator method"""
        i = 0
        ...
        for _, msg in self._ubxreader:
            try:
                if msg.identity == "NAV-PVT":
                    time = (datetime(msg.year, msg.month, msg.day, msg.hour, msg.min, msg.second).isoformat() + "Z")
                    fix = "3d" if msg.fixType == 3 else "2d" if msg.fixType == 2 else "none"
                    self.write_gpx_trkpnt(msg.lat, msg.lon, ele=msg.hMSL / 1000, time=time, fix=fix)
                if msg.identity in ("NAV-POSLLH", "NAV-HPPOSLLH"):
                    time = (date.today().isoformat() + "T" + itow2utc(msg.iTOW).isoformat() + "Z")
                    self.write_gpx_trkpnt(msg.lat, msg.lon, ele=msg.hMSL / 1000, time=time)
                i += 1
    ...
    def write_gpx_hdr(self):
        ...
        gpxtrack = (
            XML_HDR + "<gpx " + GPX_NS + ">"
            f"<metadata><link href=\"{GITHUB_LINK}\"><text>pyubx2</text></link><time>{date}</time></metadata>"
